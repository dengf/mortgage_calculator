...
fn try_create_window_with_fallback_renderer(
    ...
    // Type-ascribed on the array itself (length left as `_`, since which
    // #[cfg]s survive varies by platform) rather than on the closure
    // parameter below: with exactly one cfg-active element — the case for
    // every platform this app builds for, since only renderer-femtovg is
    // ever enabled — an array literal with no expected type infers its
    // element as that one function's own singleton zero-sized item type,
    // not a `fn(...) -> ...` pointer. Annotating only the closure
    // parameter as a fn pointer type then mismatches that inferred array
    // (E0631). Giving the array literal itself an expected fn-pointer
    // element type makes every surviving element coerce to it directly,
    // which works the same way whether one element survives or several.
    let renderer_factories: [fn(
        &Rc<SharedBackendData>,
    ) -> Result<Box<dyn WinitCompatibleRenderer>, PlatformError>; _] = [
        #[cfg(any(
